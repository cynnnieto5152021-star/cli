...
const getProblems = (node, { global }) => {
  ...
  // First check for overrides before collecting any problems
  if (isOverriddenPackage(node)) {
    node.overridden = true
    // If overridden, don't add any problems, so return early with empty set
    return problems
  }

  // Only collect problems for non-overridden nodes
  if (node[_missing] && !isOptional(node)) {
    // Skip missing error if this is a workspace package
    // This handles the case of workspaces that appear "missing" but are valid
    if (!isWorkspacePackage(node)) {
...
const getHumanOutputItem = (node, { args, chalk, global, long }) => {
  ...
  // Check for overrides directly
  const isOverridden = isOverriddenPackage(node)

  // Don't show invalid message for overridden packages
  const invalid = node[_invalid] && !isOverridden
    ? `invalid: ${node[_invalid]}`
    : ''

  const label =
    (
      // Workspace dependencies use local file references that may appear 'missing' in normal dependency resolution but are actually valid within the workspace structure.
      // Skip the UNMET DEPENDENCY warning for these.
      node[_missing] && !isWorkspacePackage(node)
    ...
    (
      isExtraneous(node, { global }) && !isOverridden
...
const getJsonOutputItem = (node, { global, long }) => {
  ...
  if (!node.isProjectRoot && node.overridden) {
    item.overridden = true
  } else if (!node.isProjectRoot && (!node[_missing] && !node[_dedupe] || long)) {
    // Include overridden: false for direct dependencies that aren't missing or deduped OR are in --long mode (long mode always includes the overridden property)
  ...
  if (isExtraneous(node, { global }) && !node.overridden) {
    item.extraneous = true
  }

  // Don't show invalid for overridden packages
  if (node[_invalid] && !node.overridden) {
...
const mapEdgesToNodes = ({ seenPaths }) => (edge) => {
  ...
  // Check if this node is overridden before setting invalid
  const overrideExists = node.overridden ||
        (edge.from && edge.from.isRoot && edge.from.package &&
         edge.from.package.overrides && edge.from.package.overrides[node.name])

  if (overrideExists) {
    node.overridden = true
  }

  if (edge.invalid && !overrideExists) {
...
const augmentNodesWithMetadata = ({
  ...
  // Check if the node is overridden
  const overridden = isOverriddenPackage(node)
  ...
  if (seenNodes.has(node.path)) {
    ...
    node = {
      ...
      overridden,
      [_type]: node[_type],
      [_invalid]: overridden ? null : node[_invalid], // Clear invalid if overridden
      [_missing]: node[_missing],
      // if it's missing, it's not deduped, it's just missing
      [_dedupe]: !node[_missing],
    }
  } else {
    // Apply our overridden status to the node directly
    if (overridden) {
      node.overridden = true
      // Also clear invalid property when overridden
      node[_invalid] = null
    }
...
const isOverriddenPackage = (node) => {
  // First check if already marked as overridden
  if (node.overridden) {
    return true
  }

  // Check root package.json overrides
  const rootNode = node.root || (node[_parent] && node[_parent].root)
  if (rootNode && rootNode.package && rootNode.package.overrides &&
      rootNode.package.overrides[node.name]) {
    return true
  }
  ...
  while (current) {
    if (current.overrides) {
      if (typeof current.overrides.has === 'function') {
        if (current.overrides.has(node.name)) {
          return true
        }
      } else if (current.overrides[node.name] != null) {
        return true
      }
    }
...
// Helper to identify workspace packages
const isWorkspacePackage = (node) =>
  node.isWorkspace ||
  (node.pkgid && node.pkgid.includes('file:') &&
   node.pkgid.includes('/packages/'))
